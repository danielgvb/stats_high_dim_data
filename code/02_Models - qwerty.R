...
data <- data %>%
  mutate(
    # CONSIDERAR CALCULO CUANDO SE TENGAN OUTLIERS!!!
...
### CAMBIAR POR VERSION DE GEORGIA
...
n_cols <- 4
n_rows <- ceiling(length(colnames(non_numeric_data)) / n_cols)

plots <- list()
for (col_name in colnames(non_numeric_data)) {
  plots[[col_name]] <- ggplot(train_data, aes_string(x = col_name, fill = "factor(default_90)")) +
...
pdf(file = paste(plots_dir, "03_EDA__non_numeric_vars_barplots.pdf", sep=""), width = n_cols * 5, height = n_rows * 3)  # Adjust dimensions
grid.arrange(grobs = plots, ncol = n_cols)
dev.off()
...
for (var in vars_to_binn) {
  ...
  if (binned_var %in% names(data_binned)) {
    ...
    plots[[binned_var]] <- ggplot(data_binned, aes_string(x = binned_var, fill = "factor(default_90)")) +
...
pdf(file = paste(plots_dir, "04_EDA__non_numeric_vars_binning.pdf", sep=""), width = n_cols * 3, height = n_rows * 2)  # Adjust dimensions
grid.arrange(grobs = plots, ncol = n_cols)
dev.off()
...
## Lasso Logistic Regression 2nd order --------------

### 1. Generate Polynomial Features --------------------------------
# Create a function to generate polynomial features
generate_poly_features <- function(data, degree = 2) {
  numeric_data <- data[, colnames(data) != "default_90"]
  poly_features <- model.matrix(~ poly(as.matrix(numeric_data), degree = degree, raw = TRUE) - 1, data = numeric_data)
  
  return(as.matrix(poly_features))
}

# Generate polynomial features for training and testing datasets
x_train_poly <- generate_poly_features(train_data, degree = 2)
y_train <- train_data$default_90

x_test_poly <- generate_poly_features(test_data, degree = 2)
y_test <- test_data$default_90
...
### 2. Perform Cross-Validation for Lasso Logistic Regression --
set.seed(123)
cv_lasso_poly <- cv.glmnet(x_train_poly, y_train, family = "binomial",
                      standarize = T,alpha = 1, nfolds = 10)
...
# Plot deviance against log(lambda)
par(mfrow= c(1,1))
plot(cv_lasso_poly, main = "Cross-Validation for Lasso Logistic Regression with 2nd order degree terms")
abline(v = log(lambda_min), col = "blue", lty = 2, lwd = 2)  # Vertical line for lambda.min
abline(v = log(lambda_1se), col = "red", lty = 2, lwd = 2)   # Vertical line for lambda.1se
legend("topright", legend = c("lambda.min", "lambda.1se"), col = c("blue", "red"), lty = 2, lwd = 2)
...
### 3. Fit the Final Model with Optimal Lambda -----------------
lasso_model_poly <- glmnet(x_train_poly, y_train, family = "binomial", alpha = 1, lambda = lambda_optimal)

### Model Evaluation---------------
predicted_probs_l <- predict(lasso_model_poly, s = lambda_optimal, newx = x_test_poly, type = "response")
lasso_metrics <- calculate_metrics(predicted_probs_l, test_data$default_90, 0.15)
lasso_metrics
...
#### (3) Coefficient Plot ----------------------------------------
coef_data <- as.data.frame(as.matrix(coef(lasso_model_poly, s = lambda_optimal)))
coef_data$Variable <- rownames(coef_data)
colnames(coef_data) <- c("Coefficient", "Variable")
coef_data <- coef_data %>% filter(Coefficient != 0 & Variable != "(Intercept)")
